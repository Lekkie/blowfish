...
public class LlllvarParseInfo extends FieldParseInfo {
	...
	throws ParseException, UnsupportedEncodingException {
		if (pos < 0) {
			throw new ParseException(String.format("Invalid LLLLVAR field %d pos %d",
                    field, pos), pos);
		} else if (pos+4 > buf.length) {
			throw new ParseException(String.format(
                    "Insufficient data for LLLLVAR header field %d pos %d", field, pos), pos);
		}
        length = decodeLength(buf, pos, 4);
		if (length < 0) {
			throw new ParseException(String.format("Invalid LLLLVAR length %d field %d pos %d",
                    length, field, pos), pos);
		} else if (length+pos+4 > buf.length) {
			throw new ParseException(String.format("Insufficient data for LLLLVAR field %d, pos %d",
                    field, pos), pos);
		}
		String _v;
        try {
            _v = length == 0 ? "" : new String(buf, pos + 4, length, getCharacterEncoding());
		...
		if (_v.length() != length) {
			_v = new String(buf, pos + 4, buf.length-pos-4, getCharacterEncoding()).substring(0, length);
		}
		if (custom == null) {
	...
	public <T> IsoValue<?> parseBinary(final int field, final byte[] buf,
		...
		if (pos < 0) {
			...
			throw new ParseException(String.format(
                    "Insufficient data for bin LLLLVAR header, field %d pos %d", field, pos), pos);
		}
		//length = ((buf[pos] & 0xf0) * 1000) + ((buf[pos] & 0x0f) * 100) + (((buf[pos + 1] & 0xf0) >> 4) * 10) + (buf[pos + 1] & 0x0f);
		length = (((buf[pos] & 0xf0) >> 4) * 1000) + ((buf[pos] & 0x0f) * 100) + (((buf[pos + 1] & 0xf0) >> 4) * 10) + (buf[pos + 1] & 0x0f);
//		length = ((buf[pos] & 0x0f) * 100) + (((buf[pos + 1] & 0xf0) >> 4) * 10) + (buf[pos + 1] & 0x0f);
		if (length < 0) {
			throw new ParseException(String.format(
                    "Invalid bin LLLLVAR length %d, field %d pos %d", length, field, pos), pos);
