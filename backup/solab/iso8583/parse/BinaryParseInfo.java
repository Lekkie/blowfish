...
//		byte[] binval = HexCodec.hexDecode(new String(buf, pos, length*2)); 
		byte [] binval = new byte[length*2];
        System.arraycopy(buf, pos, binval, 0, length*2);
//		String key = HexConverter.fromBinary2Hex((byte [])rr);
		
		if (custom == null) {
			return new IsoValue<byte[]>(type, binval, binval.length, null);
//			return new IsoValue<byte[]>(type, binval, length, null);
		} else {
