...
public class IsoMessageService {
    ...
    public MessageFactory getMessageFactory(String isoPackagerName, boolean binaryBitmap){
    ...
    public IsoMessage setCompositeChildValue(IsoMessage isoMessage, String isoPackagerName, boolean binaryBitmap, int parentFieldNo, int childFieldNo, String compositeChildVal){
    ...
    private int getCompositeFieldTotalLen(CompositeField composite, CompositeField compositeTemplVal, IsoValue compositeTemplChildField){
        int totalLen = 0;
        int j = 0;
        /*
    ...
    public IsoMessage setValue(IsoMessage isoMessage, String isoPackagerName, boolean binaryBitmap, int fieldNo, String val) {
    ...
    public void sendResponse(ISO8583SrcNode iso8583SrcNode, ChannelHandlerContext ctx, IsoMessage isoReq, String packagerName, boolean binaryBitmap, String responseCode){
    ...
    public IsoMessage returnResponse(IsoMessage isoMessage, String packagerName, boolean binaryBitmap, String respCode){
    ...
    public IsoMessage createResponse(IsoMessage request, String isoPackagerName, boolean binaryBitmap){
    ...
    public String getCompositeFieldValue(IsoMessage isoMessage, int parentFieldNo, int childFieldNo){
        try{
    ...
    public String getFieldValue(IsoMessage isoMessage, int fieldNo){
        try{
    ...
    public boolean isRequest(IsoMessage isoMessage){
    ...
    // https://en.wikipedia.org/wiki/ISO_8583
    public boolean isResponse(IsoMessage isoMessage){
        String type = String.format("%04X", isoMessage.getType());
        int len = type.length();
        if(type.length() > 2){
            String last2Xters = type.substring(len - 2, len);
    ...
    public boolean isEchoRequest(IsoMessage isoMessage){
        String type = String.format("%04X", isoMessage.getType());
        return "0800".equals(type) || "0801".equals(type);
    }

    public boolean isEchoResponse(IsoMessage isoMessage){
        String type = String.format("%04X", isoMessage.getType());
        return "0810".equals(type);
    }


    public String getResponseType(String requestType){
    ...
    public String getRequestType(String responseType){
    ...
    public String getTranType(IsoMessage isoMessage){
        try{
            String proc = getFieldValue(isoMessage, f3);
            return getTranType(proc);
        }
        catch(Exception ex){
    ...
    public String getTranType(String proc){
        try{
    ...
    public String getFromAccountType(IsoMessage isoMessage){
        try{
            String proc = getFieldValue(isoMessage, f3);
            return getFromAccountType(proc);
        }
        catch(Exception ex){
    ...
    public String getFromAccountType(String proc){
        try{
    ...
    public String getToAccountType(IsoMessage isoMessage){
        try{
            String proc = getFieldValue(isoMessage, f3);
            return getToAccountType(proc);
        }
        catch(Exception ex){
    ...
    public String getToAccountType(String proc){
        try{
    ...
    public String getOriginalDataElement(IsoMessage isoMessage){
        ...
        acquiringInstId = stringService.leftPad(acquiringInstId, 11, '0');
        String forwardingInstId = getFieldValue(isoMessage, f33);
        forwardingInstId = forwardingInstId == null ? "" : forwardingInstId;
        forwardingInstId = stringService.leftPad(forwardingInstId, 11, '0');
    ...
    public Long generateTMSKey(IsoMessage isoMessage, TMSKeyOrigDataElemService tmsKeyOrigDataElemService){
        ...
        Optional<TMSKeyOrigDataElem> tmsKeyOrigDataElemOptional = tmsKeyOrigDataElemService.findByOriginalDataElement(originalDataElement);
        if(!tmsKeyOrigDataElemOptional.isPresent()){
            logger.error("receiveRequestFromRemote: TMS key not found for this message");
            return 0L;
        }
        tmsKeyOrigDataElem = tmsKeyOrigDataElemOptional.get();
        return  tmsKeyOrigDataElem.getTmsKeyOrigDataElemId();
    }

    public Long getTMSKey(IsoMessage isoMessage, TMSKeyOrigDataElemService tmsKeyOrigDataElemService){
        String originalDataElement = getOriginalDataElement(isoMessage);
        Optional<TMSKeyOrigDataElem> tmsKeyOrigDataElemOptional = tmsKeyOrigDataElemService.findByOriginalDataElement(originalDataElement);
        return tmsKeyOrigDataElemOptional.isPresent() ? tmsKeyOrigDataElemOptional.get().getTmsKeyOrigDataElemId() :  0L;
    }
