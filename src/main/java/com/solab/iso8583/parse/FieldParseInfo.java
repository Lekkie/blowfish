...
public abstract class FieldParseInfo {
	...
	public static FieldParseInfo getInstance(IsoType t, int len, String encoding) {
		...
		} else if (t == IsoType.LLLLLVAR) {// Added by Lekan Omotayo
			fpi = new LllllvarParseInfo();
		} else if (t == IsoType.LLLLLLVAR) { // Added by Lekan Omotayo
    ...
    protected int decodeLength(byte[] buf, int pos, int digits) throws UnsupportedEncodingException {
        if (forceStringDecoding) {
        ...
        }else if (digits == 6) { // Added by Lekan Omotayo
            return ((buf[pos] - 48) * 100000)
					+ ((buf[pos + 1] - 48) * 10000)
					+ ((buf[pos + 2] - 48) * 1000)
					+ ((buf[pos + 3] - 48) * 100)
					+ ((buf[pos + 4] - 48) * 10)
            ...
            return ((buf[pos] - 48) * 100)
					+ ((buf[pos + 1] - 48) * 10)
					+ (buf[pos + 2] - 48);
        } else {
            return ((buf[pos] - 48) * 10)
					+ (buf[pos + 1] - 48);
        }
    }
