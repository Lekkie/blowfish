...
public class MessageFactory<T extends IsoMessage> {
	...
	public T newMessage(int type) { // Modified by Lekan Omotayo
		T m;
	...
	public T createResponse(T request) { // Modified by Lekan Omotayo
		...
		resp.setForceStringEncoding(forceStringEncoding);
		...
		*/
		//else {
			for (int i = 2; i < 128; i++) {
				if (request.hasField(i)) {
					//resp.setField(i, request.getField(i).clone());
					if(request.getField(i).getValue() instanceof CompositeField){
						CompositeField reqComposite = (CompositeField) request.getField(i).getValue();
						CompositeField respCompositeField = new CompositeField();
						IsoValue compositeTempl = templ.getField(i);
						...
						for(IsoValue index:values){
							if(reqComposite.getField(j)!= null){
								String reqCompositeChildVal = reqComposite.getObjectValue(j);
								IsoValue compositeTemplChildField = compositeTemplVal.getField(j);
								IsoType compositeChildType = compositeTemplChildField.getType();
								if(!compositeChildType.needsLength()){
									int reqCompositeChildValLen = reqCompositeChildVal.length();
									totalLen += reqCompositeChildValLen;
									respCompositeField.addValue(j, new IsoValue<String>(compositeChildType, reqCompositeChildVal, reqCompositeChildValLen)); //LLVAR
								}
								else{
									int compositeChildLen = compositeTemplChildField.getLength();
									totalLen += compositeChildLen;
									respCompositeField.addValue(j, new IsoValue<String>(compositeChildType, reqCompositeChildVal, compositeChildLen));
								}
							}
							j++;
						}
						if(totalLen > 0)
							resp.setValue(i, respCompositeField, respCompositeField, compositeTempl.getType(), totalLen);
					}
					else{
						IsoType templFieldType = templ.getField(i).getType();
						if(!templFieldType.needsLength()){
							String value = request.getObjectValue(i);
							resp.setValue(i,  value, templFieldType, value.length());

						}else{
							IsoValue field = templ.getField(i);
							resp.setValue(i, request.getObjectValue(i), templFieldType, field.getLength());
						}
					}
				}
					...
					if (ignoreLast && pos >= buf.length && i.intValue() == index.get(index.size() -1)) {
						...
						if (val != null) {
							...
							if (val.getType() == IsoType.LLVAR || val.getType() == IsoType.LLBIN) {
								...
								pos += 2;
							}else if (val.getType() == IsoType.LLLLLVAR // Added by Lekan Omotayo
									|| val.getType() == IsoType.LLLLLLVAR) { // Added by Lekan Omotayo
								pos += 3;
							}
						}
					}
				...
				if (bs.get(i - 1)) {
					if (ignoreLast && pos >= buf.length && i.intValue() == index.get(index.size() -1)) {
						...
						if (val.getType() == IsoType.LLVAR || val.getType() == IsoType.LLBIN) {
							...
							pos += 5;
						} else if (val.getType() == IsoType.LLLLLLVAR) { // Added by Lekan Omotayo
							pos += 6;
						}
					}
				}
